...
if (!process.env.API_KEY) {
  console.error("FATAL ERROR: API_KEY is not defined in the environment variables. Please create a .env file with your API_KEY.");
  process.exit(1);
}
...
if (!GOOGLE_CLIENT_ID) {
    console.error("FATAL ERROR: GOOGLE_CLIENT_ID is not set in environment variables. Google Sign-In will not function on the backend. Please set it in your .env file.");
}
...
let users = []; // { id, username, passwordHash (optional), email (optional), googleId (optional), name (optional, from Google) }
...
app.post('/auth/register', async (req, res) => {
  try {
    ...
    res.status(201).json({ message: 'User created successfully', token, user: {id: newUser.id, username: newUser.username } });
  } catch (error) {
...
app.post('/auth/google/verify', async (req, res) => {
    if (!GOOGLE_CLIENT_ID) { 
        ...
        return res.status(500).json({ message: "Google Sign-In is not configured on the server. Administrator check server logs." });
    }
    try {
...
app.get('/api/user/me', authenticateToken, (req, res) => {
    ...
    res.json({ user: { id: userProfile.id, username: userProfile.username, email: userProfile.email, name: userProfile.name } });
});
...
app.post('/api/analyze-pdf', authenticateToken, async (req, res) => {
  try {
    const {
      base64PdfData, pdfMimeType, pdfFileName, documentType,
    ...
    const requestOptions = { documentType, simplifiedExplanation, pageByPageExplanation, pageNumbers, explainWithAnalogy, analogyTopic, glossaryBuilder, quizGenerator, numQuestions, multilingualMode, language, chatSupport, initialQuestion};
...
app.post('/api/follow-up-chat', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (priorChatHistory && priorChatHistory.length > 0) {
        console.log(`Follow-up: Last prior message - Role: ${priorChatHistory[priorChatHistory.length - 1].role}, Text: "${priorChatHistory[priorChatHistory.length - 1].text.substring(0, 50)}..."`);
    }
    ...
    const validMessages = priorChatHistory.filter(msg => 
...
Keep your answer concise, informative, and directly related to the document content.`;
    ...
    const result = await ai.models.generateContent({
        model: "gemini-2.5-flash-preview-04-17",
        contents: { parts: [filePart, textPart] },
        systemInstruction: "You are an academic assistant helping a user understand a PDF document. Provide clear, concise, and accurate responses based on the document content."
    });
    ...
    if (error.details) { // For some GoogleGenAIError types
